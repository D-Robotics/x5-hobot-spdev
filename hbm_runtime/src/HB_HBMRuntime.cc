...
HB_HBMRuntime::HB_HBMRuntime(const string& model_file)
{
    ...
    if (model_file.substr(model_file.find_last_of('.') + 1) != "bin") {
...
void HB_HBMRuntime::FreeTensorMem(std::vector<hbDNNTensor>& tensors) {
    for (auto& tensor : tensors) {
        if (tensor.sysMem[0].virAddr != nullptr) {
            ...
            HB_CHECK_SUCCESS(hbSysFreeMem(&(tensor.sysMem[0])) , "FreeTensorMem() hbSysFreeMem failed");


            // Reset memory pointers to avoid dangling references.
            tensor.sysMem[0].virAddr = nullptr;
            tensor.sysMem[0].phyAddr = 0;
        }
    }
}
                                              ...
                                              hbDNNTensor &input_tensor, py::array & input_array)
{
    ...
    if (properties.tensorType == HB_DNN_IMG_TYPE_NV12 || 
        properties.tensorType == HB_DNN_IMG_TYPE_NV12_SEPARATE) {
        
        auto aligned_size = properties.alignedByteSize;
        HB_CHECK_SUCCESS(hbSysAllocCachedMem(&input_tensor.sysMem[0], aligned_size),
                        "hbSysAllocCachedMem() failed");

        auto copy_size = std::min(static_cast<pybind11::ssize_t>(aligned_size), input_array.nbytes());
        memcpy(input_tensor.sysMem[0].virAddr, input_array.data(), copy_size);
    ...
    HB_CHECK_SUCCESS(hbSysFlushMem(&input_tensor.sysMem[0], HB_SYS_MEM_CACHE_CLEAN), 
                                          ...
                                          std::unordered_map<std::string, py::array>& input_arrays)
{
    ...
    for (int32_t input_tensor_idx{0}; input_tensor_idx < input_counts[model_name]; input_tensor_idx++) {
        ...
        // std::cout << "PrepareInputTensor():tensorType: " << input_properties.tensorType << std::endl;
        if(input_properties.tensorType != HB_DNN_IMG_TYPE_NV12)
        {
            ...
            PrepareInputStride(model_name, input_name, input_properties.validShape, reinterpret_cast<int64_t*>(input_properties.stride), input_array);
        }
...
int32_t HB_HBMRuntime::PrepareOutputTensor(std::string model_name, std::vector<hbDNNTensor> & output_tensors)
{
    ...
    for (int32_t idx{0}; idx < output_counts[model_name]; idx++) {
        ...
        HB_CHECK_SUCCESS(hbSysAllocCachedMem(&tensor.sysMem[0], tensor_props.alignedByteSize) , 
                            ...
                            std::vector<hbDNNTensor> & output_tensors)
{
    for (int32_t output_tensor_idx{0}; output_tensor_idx < output_counts[model_name]; output_tensor_idx++) {
        ...
        py::array np_array = MakeNumpyArrayWithCapsule(dtype, shape, strides, output_tensors[output_tensor_idx].sysMem[0]);
                 ...
                 std::vector<hbDNNTensor>& output_tensors)
{
    ...
    hbDNNTensor* input_ptr = input_tensors.data();
    HBDNN_CHECK_SUCCESS(hbDNNInfer(&task_handle, 
        &output_ptr,
        input_tensors.data(),
        dnn_handle,
        &infer_ctrl_param),
    ...
    for (int output_tensor_idx = 0; output_tensor_idx < output_tensors.size(); output_tensor_idx++) {
        HB_CHECK_SUCCESS(hbSysFlushMem(&output_tensors[output_tensor_idx].sysMem[0], HB_SYS_MEM_CACHE_INVALIDATE) , "hbSysFlushMem() failed");


        std::string tensor_name = output_names[model_name][output_tensor_idx];
        hbDNNTensorProperties &output_properties = output_tensor_properties[model_name][tensor_name];
        HBDNN_CHECK_SUCCESS(hbDNNGetOutputTensorProperties(&output_properties, dnn_handle, output_tensor_idx),
                   ...
                   const ExtraArgs& extra_args)
{
    ...
    try {
        auto result = InferAllModels(multi_input_tensors);
        // std::cout << "=== C++ Debug: InferAllModels succeeded ===" << std::endl;
        return result;
    } catch (const std::exception& e) {
        std::cout << "=== C++ Debug: InferAllModels failed with exception: " << e.what() << " ===" << std::endl;
        throw;
    }
}
